...
def _check_coinbase() -> CheckResult:
    """Check Coinbase Exchange public market-data reachability."""
    try:
        import requests
        ...
        if resp.ok and isinstance(data, list):
            return CheckResult(name="Coinbase API", status="ready", message="reachable", impact="")
        return CheckResult(
            name="Coinbase API",
            status="error",
...
def _check_exa() -> CheckResult:
    """Report Exa configuration for web and market research tools."""
    api_key = get_env_config().data.exa_api_key.strip()
    if not api_key or api_key == "your-exa-api-key":
        return CheckResult(
            name="Exa Research",
            status="not_configured",
            message="EXA_API_KEY not set",
            impact="web_search and read_url tools unavailable",
        )
    return CheckResult(name="Exa Research", status="ready", message="API key configured", impact="")

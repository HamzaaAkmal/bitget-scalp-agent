...
_CRYPTO_ONLY_DISABLED_TOOL_NAMES = {
    "alpha_bench",
    "alpha_compare",
    "alpha_zoo",
    "factor_analysis",
    ...
    for cls in _discover_subclasses():
        try:
            if cls.name in _SHELL_TOOL_NAMES and not include_shell_tools:
                ...
                continue
            if cls.name in _CRYPTO_ONLY_DISABLED_TOOL_NAMES:
                logger.info("Tool %s disabled by crypto-only product policy", cls.name)
                continue
...
def build_swarm_registry(
    ...
    *,
    ...
    file or env var; callers of swarm entry points cannot inject MCP server

...
class DataSourceSettingsResponse(BaseModel):
    ...
    exa_api_key_hint: Optional[str] = None
...
EXA_API_KEY_PLACEHOLDERS = {"", "your-exa-api-key"}
...
def _read_settings_env_values() -> Dict[str, str]:
    ...
    Prefer the canonical user config, while filling missing values from the
    legacy package-local config. If neither exists, use ``agent/.env.example``
    for display defaults only.
    """
    host = _host()
    ...
    if legacy_env_path.exists():
        values.update(read_env(legacy_env_path))
    if env_path.exists():
...
def _build_data_source_settings_response(
    ...
    exa_key = env_values.get("EXA_API_KEY", "")
    exa_configured = host._is_configured_secret(exa_key, EXA_API_KEY_PLACEHOLDERS)
    ...
    return DataSourceSettingsResponse(
        ...
        exa_api_key_configured=exa_configured,
        exa_api_key_hint=None,
...
def register_settings_routes(
    ...
    async def update_data_source_settings(payload: UpdateDataSourceSettingsRequest):
        ...
        if payload.clear_exa_api_key:
            updates["EXA_API_KEY"] = ""
        elif payload.exa_api_key is not None and payload.exa_api_key.strip():
            api_key = payload.exa_api_key.strip()
            updates["EXA_API_KEY"] = (
                api_key
                if host_ref._is_configured_secret(api_key, EXA_API_KEY_PLACEHOLDERS)
                else ""
            )
        elif "EXA_API_KEY" in current_values:
            updates["EXA_API_KEY"] = current_values["EXA_API_KEY"]
        ...
        if updates:
            ...
            exa_key = updates.get("EXA_API_KEY", "").strip()
            if host_ref._is_configured_secret(exa_key, EXA_API_KEY_PLACEHOLDERS):
